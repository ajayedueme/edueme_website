...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	if($_FILES['txtDocument']['name'] !=""){
		...
		move_uploaded_file($tempimagename2,'../company-logos/'.$imagename2);
	}else{
		$thumbfile2="";
	}
	$sqladd = "INSERT INTO `tbl_mjcet_companies`(`pubDocument`, `pubOrder`, `status`, `addOn`) VALUES ('{$thumbfile2}', '{$txtOrder}', '1', Now())";
	
	$res_add =  mysqli_query($con,$sqladd);
	
	header('Location:companies.php?msg=4');
	exit();
