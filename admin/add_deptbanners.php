...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	if($department == 'Other'){
		$menusulg = htmlspecialchars(addslashes($_POST['menusulg1']));
	}else{
	...
	if($_FILES['txtDocument']['name'] !=""){
		...
		move_uploaded_file($tempimagename2,'../dept-banners/'.time().$imagename2);
	}else{
		$thumbfile2="";
	}
	$sqladd = "INSERT INTO `tbl_mjcet_banner`(`deptMenu`, `deptBanner`, `menuSulg`, `banOrder`, `status`, `addOn`) VALUES ('{$department}', '{$thumbfile2}', '{$menusulg}', '{$txtOrder}', '1', Now())";
	$res_add =  mysqli_query($con,$sqladd);
	
	header('Location:deptbanners.php?msg=4');
	exit(); 
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Add Banner</h1>
      <ol class="breadcrumb">
	  <li><a href="deptbanners.php">Banners List</a></li>
			...
			<div class="panel panel-default">  <br>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					...
					<div class="form-group">
						...
						<select class="form-control" name="menusulg1" id="menusulg1" style="display:none">
							<option value="placement-home">Placement</option>
							<option value="home-library">Library</option>
							<option value="r-and-d-home">R and D</option>
...
$('#department').change(function(){
	var dval = (this.value);
	if(dval == 'Other'){
		$("#menusulg1").css('display','block');
		$("#menusulg").css('display','none');
	}else{
		$("#menusulg").css('display','block');
		$("#menusulg1").css('display','none');
		$.ajax({
			url:'ajax.php',
			type:'post',
