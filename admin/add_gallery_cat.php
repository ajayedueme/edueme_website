...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$cattype = htmlspecialchars(addslashes($_POST['txtCatType']));
	$txtimgtitle = htmlspecialchars(addslashes($_POST['txtimgtitle']));
	$title = strtolower($_POST['txturltitle']);
	$Sulg = preg_replace('/[ ,&,-,.]+/', '-', trim($title));
	if($cattype=='placement-gallery'){
		$txtSulg = 'placement-'.$Sulg;
	}else{
		$txtSulg = 'gallery-'.$Sulg;
	}
	...
	if($_FILES['txtThumb']['name'] !=""){
		$thumbfile2= time().$_FILES['txtThumb']['name'];
		$imagename2= $_FILES['txtThumb']['name'];
		$tempimagename2= $_FILES['txtThumb']['tmp_name'];
		move_uploaded_file($tempimagename2,'../'.$cattype.'/'.time().$imagename2);
	}else{
		$thumbfile2="";
	}
	$sqladd = "INSERT INTO `tbl_mjcet_gallery_cat`(`catType`, `catThumbnail`, `catTitle`, `catSulg`, `catOrder`, `status`, `addOn`) VALUES ('{$cattype}', '{$thumbfile2}', '{$txtimgtitle}', '{$txtSulg}', '{$txtOrder}', '1', Now())";
	$res_add =  mysqli_query($con,$sqladd);
	
	header('Location:gallery_cat.php?msg=4');
	exit();
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Add</h1>
      <ol class="breadcrumb">
	  <li><a href="gallery_cat.php">Gallery Category List</a></li>
			...
			<div class="panel panel-default">  <br>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					...
					<div class="form-group">
						...
						<select class="form-control" name="txtCatType" id="txtCatType">
							<option value="">Select</option>
							<option value="gallery-placement">Placement Gallery</option>
					...
					<div class="form-group">
						<label for="input1" class="form-label">Gallery URL Name: </label>
						<input type="text" class="form-control" name="txturltitle" id="txturltitle">
					</div>
					<div class="form-group">
						<label for="input1" class="form-label">Thumbnail: </label>
