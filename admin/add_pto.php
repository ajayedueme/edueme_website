...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	$txtttle = htmlspecialchars(addslashes($_POST['txtttle']));	
	$txtname = htmlspecialchars(addslashes($_POST['txtname']));
	$txtEducation = htmlspecialchars(addslashes($_POST['txtEducation']));
	$txttel = htmlspecialchars(addslashes($_POST['txttel']));
	$txtFax = htmlspecialchars(addslashes($_POST['txtFax']));
	$txtMobile = htmlspecialchars(addslashes($_POST['txtMobile']));
	$txtemail = htmlspecialchars(addslashes($_POST['txtemail']));
	
	
	
	
	$sqladd = "INSERT INTO `tbl_mjcet_pto`(`ptoTitle`, `ptoName`, `ptoEdu`, `ptoTel`, `ptoFax`, `ptoMobile`, `ptoEmail`, `status`, `addOn`) Values ('{$txtttle}', '{$txtname}', '{$txtEducation}', '{$txttel}', '{$txtFax}', '{$txtMobile}', '{$txtemail}', '1', Now())";
	$res_add =  mysqli_query($con,$sqladd);
	
	header('Location:pto.php?msg=4');
	exit();
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Add</h1>
      <ol class="breadcrumb">
	  <li><a href="pto.php">P & TO List</a></li>
			...
			<div class="panel panel-default">  <br>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" name="txtttle" id="txtttle"  >
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label for="input1" class="form-label">Education: </label>
						<input type="text" class="form-control" name="txtEducation" id="txtEducation" >
					</div>
					<div class="form-group">
						<label for="input1" class="form-label">Tel: </label>
						<input type="text" class="form-control" name="txttel" id="txttel" >
					</div>
					<div class="form-group">
						<label for="input1" class="form-label">Fax: </label>
