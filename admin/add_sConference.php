...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$txtPage = htmlspecialchars(addslashes($_POST['txtPage']));
	$txtOrder = htmlspecialchars(addslashes($_POST['txtOrder']));
	
	if($_FILES['txtBrochure']['name'] !=""){
		$thumbfile2= $_FILES['txtBrochure']['name'];
		$imagename2= $_FILES['txtBrochure']['name'];
		$tempimagename2= $_FILES['txtBrochure']['tmp_name'];
		move_uploaded_file($tempimagename2,'../documents/'.$imagename2);
	}else{
		$thumbfile2="";
	}
	
	$sqladd = "INSERT INTO `tbl_mjcet_student_conference`(`cName`, `cPage`, `cBrochure`, `corder`, `status`, `addOn`) VALUES ('{$txtName}', '{$txtPage}', '{$thumbfile2}', '{$txtOrder}', '1', Now())";
	$res_add =  mysqli_query($con,$sqladd);
	$lattid = mysqli_insert_id($con);

	header('Location:sConference.php?msg=4');
	exit();
  ...
  <!-- Start Page Header -->
	<div class="page-header">
		<h1 class="title">Add</h1>
		  <ol class="breadcrumb">
		  <li><a href="sConference.php">Student Conference List</a></li>
					...
					<div class="panel-body">
						<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
							...
							<div class="form-group">
								<label for="input1" class="form-label">Conference Name: </label>
								<input type="text" class="form-control" name="txtName" id="txtName" required>
							</div>
							<div class="form-group">
								<label for="input1" class="form-label">Conference Page: </label>
								<input type="text" class="form-control" name="txtPage" id="txtPage" >
							</div>
							<div class="form-group">
								<label for="input1" class="form-label">Brochure: </label>
								<input type="file" class="form-control" name="txtBrochure" id="txtBrochure" >
							</div>
							<div class="form-group">
