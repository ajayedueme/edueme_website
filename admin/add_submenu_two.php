...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$sqladd = "INSERT INTO `tbl_mjcet_first_submenu`(`menuType`, `menuSulg`, `menuName`, menuTitle, `menuDocument`, `menuContent`, menuContentTwo,`menuImage`, `menuOrder`, `status`, `addOn`) VALUES ('{$txtMenuType}', '{$txtUrl}', '{$txtMenuname}', '{$menuTitle}', '{$thumbfile1}', '{$txtMenuContent}', '{$txtMenuContent2}', '{$imagename2}', '{$txtMenuOrder}', '1', Now())";
	$res_add =  mysqli_query($con,$sqladd);
		
	header('Location:submenu.php?msg=4');
	exit();
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Sub Add Menu</h1>
      <ol class="breadcrumb">
			...
			<div class="panel panel-default">  <br>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					<div class="form-group">
						...
						<select class="form-control" name="txtMenuType" id="txtMenuType" >
							...
							<?php 
								...
								$mainmenu =  mysqli_query($con,"SELECT * FROM tbl_mjcet_first_menu ORDER BY menuOrder");
								while($res_mainmenu= mysqli_fetch_object($mainmenu)){
							
							?>
							<option value="<?php echo $res_mainmenu->menuId?>" ><?php echo $res_mainmenu->menuName?></option>
								<?php } ?>
						</select>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<textarea id="elm1" name="txtMenuContent2"></textarea>
					</div>
					<div class="form-group">
