...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	if($_POST['txtSubMen'] != ''){
		$txtUrl = htmlspecialchars(addslashes($_POST['txtSubMen']));
	}else{
		$txtUrl = htmlspecialchars(addslashes($_POST['txtMenuname']));
	}
	...
	$sqladd = "INSERT INTO `tbl_mjcet_side_menu_content`(`menuSulg`, `menuName`, `menuDocument`, `menuOrder`, `status`, `addOn`) VALUES ('{$txtUrl}', '{$menuTitle}', '{$thumbfile1}', '{$txtMenuOrder}', '1', Now())";
	$res_add =  mysqli_query($con,$sqladd);
		
	header('Location:submenucontent.php?msg=4');
	exit();
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Add </h1>
      <ol class="breadcrumb">
	  <li><a href="submenucontent.php">List</a></li>
			...
			<div class="panel panel-default">  <br>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					<div class="form-group">
						<label for="input1" class="form-label">Main Menu: </label>
						<select class="form-control" name="txtMenuname" id="txtMenuname" >
							...
							?>
							<option value="<?php echo $res_mainmenu->menuSulg?>" data-test='<?php echo $res_mainmenu->menuId?>'><?php echo $res_mainmenu->menuName?></option>
								<?php } ?>
						</select>
					</div>
					<div class="form-group">
						<label for="input1" class="form-label">Sub Menu: </label>
						<select class="form-control" name="txtSubMen" id="txtSubMen" >
							<option value="">Select</option>
						</select>
					</div>
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" name="menuTitle" id="menuTitle" required>
					</div>
					<div class="form-group">
...
$('#txtMenuname').change(function(){
	var smid = $(this).find(':selected').data('test');
	$.ajax({
		url:'ajax.php',
		type:'post',
		data:{'action':"SideMenu",'sid':smid},
