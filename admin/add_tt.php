...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	if($_FILES['txtDocument']['name'] !=""){
		$thumbfile1= $_FILES['txtDocument']['name'];
		$imagename= $_FILES['txtDocument']['name'];
		$tempimagename= $_FILES['txtDocument']['tmp_name'];
		move_uploaded_file($tempimagename,'../documents/'.$imagename);
	}else{
		$thumbfile1="";
	}
	
	if($_FILES['txtDocumentTwo']['name'] !=""){
		$thumbfile2= $_FILES['txtDocumentTwo']['name'];
		$imagename2= $_FILES['txtDocumentTwo']['name'];
		$tempimagename2= $_FILES['txtDocumentTwo']['tmp_name'];
		move_uploaded_file($tempimagename2,'../documents/'.$imagename2);
	}else{
		$thumbfile2="";
	}
	$sqladd = "INSERT INTO `tbl_mjcet_timetable`(`pubSulg`, `pubName`, `pubDocument`, `pubDocument_two`, `pubOrder`, `status`, `addOn`) VALUES ('{$publication}', '{$txtname}', '{$thumbfile1}', '{$thumbfile2}', '{$txtOrder}', '1', Now())";
	$res_add =  mysqli_query($con,$sqladd);
	
	header('Location:tt.php?msg=4');
	exit();
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Add Time Table</h1>
      <ol class="breadcrumb">
	  <li><a href="tt.php">Time Table List</a></li>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					...
					<div class="form-group">
						...
						<select class="form-control" name="publication" id="publication">
							...
							<?php 
								$pub = mysqli_query($con, "SELECT * FROM `tbl_mjcet_megha_menu_dept_section` WHERE menuName = 'Time Table' AND status='1'"); 
								while($res_pub = mysqli_fetch_object($pub)){
							?>
								<option value="<?php echo $res_pub->menuSulg; ?>"><?php echo html_entity_decode($res_pub->mainMenuName); ?></option>
							<?php } ?>
							<?php 
								$coursePub = mysqli_query($con, "SELECT * FROM `tbl_mjcet_megha_menu_courses` WHERE menuName = 'Time Table' AND status='1'"); 
								while($res_coursePub = mysqli_fetch_object($coursePub)){
							?>
								<option value="<?php echo $res_coursePub->menuSulg; ?>"><?php echo html_entity_decode($res_coursePub->mainMenuName); ?></option>
							<?php } ?>
					...
					<div class="form-group">
						<label for="input1" class="form-label">Second Document: </label>
						<input type="file" class="form-control" name="txtDocumentTwo" id="txtDocumentTwo" >
					</div>
					<div class="form-group">
