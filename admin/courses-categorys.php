...
$sqlfaq = "SELECT * FROM tbl_course_categories";
  ...
  <!-- Start Page Header -->
	...
	<div class="page-header" style="width: 100%; overflow: hidden;">
		<div style="width: 600px; float: left;"> 
			<h1 class="title">Course Categories List</h1>
		</div>
		<div style="float: right;"> 
			<a class="btn btn-default" href="add-courses-category.php">Add</a>
		</div>
            ...
            <table id="example0" class="table display">
                ...
                <?php $i=1; while($row_faq = mysqli_fetch_object($res_faq)){?>
                    <tr>
                        ...
                        <td width="28%"><?php echo stripslashes($row_faq->title);?></td>
                        
                        <td width="10%"><?php echo stripslashes($row_faq->cat_order);?></td>
                        ...
                        <td width="10%">
							<a href="edit-courses-category.php?eid=<?php echo $row_faq->id;?>">Edit</a> | 
...
$(document).ready(function() {
	...
	$('.delete').click(function(e) {
		...
		if (r == true) {
			...
			$.ajax({
				...
				type:'post',
				data:{'action':"Delete",'tblname':'tbl_course_categories','dltid':'id','dltval':dltid},
	...
	$('select[name^="statchnage"]').change(function(){
		...
		$.ajax({
			url:'ajax.php',
			type:'post',
			data:{'actions':'update','tablename_status':'tbl_course_categories','statuscolumn':'status','status':status_value,'idcolumn':'id','id':cahnge_id},
			success:function(data){
				location.reload();
			}
		}); 
