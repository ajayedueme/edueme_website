...
$sqlfaq = "SELECT * FROM tbl_mjcet_megha_menu_courses WHERE mainMenuId = '1'";
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Courses Menu List</h1>
      <ol class="breadcrumb">
    </ol>
                ...
                <?php $j=1; while($row_faq = mysqli_fetch_object($res_faq)){?>
                    <tr>
                        ...
                        <td width="10%" style="align:center">
							<a href="edit_courses_menu.php?eid=<?php echo $row_faq->tlmid;?>">Edit</a> | 
							<a class="delete" href="javascript:void(0)" data-id="<?php echo $row_faq->tlmid;?>">Delete</a>
...
$(document).ready(function() {
	...
	$('.delete').click(function(e) {
		...
		if (r == true) {
			...
			$.ajax({
				...
				type:'post',
				data:{'action':"Delete",'tblname':'tbl_mjcet_megha_menu_courses','dltid':'tlmid','dltval':dltid},
	...
	$('select[name^="statchnage"]').change(function(){
		...
		$.ajax({
			url:'ajax.php',
			type:'post',
			data:{'actions':'update','tablename_status':'tbl_mjcet_megha_menu_courses','statuscolumn':'status','status':status_value,'idcolumn':'tlmid','id':cahnge_id},
			success:function(data){
				location.reload();
			}
		}); 
