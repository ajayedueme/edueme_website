...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$txtNumber = htmlspecialchars(addslashes($_POST['txtNumber']));
	$txtname = htmlspecialchars(addslashes($_POST['txtname']));
	$txtOrder = htmlspecialchars(addslashes($_POST['txtOrder']));
	
	$sqlupdate = "UPDATE `tbl_mjcet_counter` SET  `pubName`='{$txtname}', `pubNumber`='{$txtNumber}',`pubOrder`='{$txtOrder}', `status`='1' , `addOn`=now() WHERE pubId = '{$eid}'";
	$res_add =  mysqli_query($con,$sqlupdate);
	header('Location:counter.php?msg=4');
	exit();
	
}else{
	echo $sql = "SELECT * FROM tbl_mjcet_counter WHERE pubId = '{$eid}'";
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Add Counter</h1>
      <ol class="breadcrumb">
	  <li><a href="counter.php">Counter List</a></li>
		...
		<div class="col-md-12 col-lg-6">
			<div class="panel panel-default">  <br>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					...
					<div class="form-group">
						<label for="input1" class="form-label">Counter: </label>
						<input type="text" class="form-control" name="txtNumber" id="txtNumber" value="<?php echo $result->pubNumber?>">
					</div>
					<div class="form-group">
