...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$txtSectionMenuname = htmlspecialchars(addslashes($_POST['txtSectionMenuname']));
	...
	$menuId = htmlspecialchars(addslashes($_POST['menuId']));
	
		
	$sqlupdate = "UPDATE `tbl_mjcet_megha_menu_courses` SET `menuName`='{$txtSectionMenuname}', `menuContent`='{$txtMenuContent}', `menuSulg`='{$txtURL}',`menuOrder`='{$txtOrder}'";
	...
	$sqlupdate .= " WHERE tlmid = '{$menuId}'";
	mysqli_query($con,$sqlupdate);
	header('Location:courses_menu.php?msg=4');
	exit();
}else{
	$sql = "SELECT * FROM tbl_mjcet_megha_menu_courses WHERE tlmid = '{$eid}'";
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Update Department Menu</h1>
      <ol class="breadcrumb">
	  <li><a href="section_menu.php">Department Menu List</a></li>
        <li class="active">Update Department Menu</li>
    </ol>
	...
	?>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					<div class="form-group">
						...
						<input type="text" class="form-control" name="txtSectionMenuname" id="txtSectionMenuname" required value="<?php echo $result->menuName?>">
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<a href='../documents/<?php echo $result->menuDocument?>' target="_blank"><?php echo $result->menuDocument?> &nbsp;&nbsp;</a> <a href="delete_records.php?action=MenuUpdate&id=<?php echo $result->tlmid?>&url=d">Delete</a>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" name="txtOrder" id="txtOrder" required value="<?php echo $result->menuOrder?>">
					</div>
					<input type="hidden" name="menuId" value="<?php echo $result->tlmid?>">
