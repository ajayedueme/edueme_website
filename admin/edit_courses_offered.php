...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$sqlupdate = "UPDATE `tbl_mjcet_courses_offered` SET `deptid`='{$txtDeptcat}', `name`='{$txtName}',`sulg`='{$txturl}',`corder`='{$txtOrder}' , `status`='1' , `addOn`=now()";
	...
	if($_FILES['txtThumb']['name'] !=""){
		...
		$sqlupdate .= ", icon = '{$thumbfile1}'";
	}
	
	$sqlupdate .= " WHERE coid = '{$eid}'";
	mysqli_query($con,$sqlupdate);
	header('Location:courses_offered.php?msg=4');
	exit();
	
}else{
	$sql = "SELECT * FROM tbl_mjcet_courses_offered WHERE coid = '{$eid}'";
								...
								<select class="form-control" name="txtDeptcat" id="txtDeptcat" required>
									...
									<?php
										...
										while($res_dept =  mysqli_fetch_object($dept)){ ?>
											...
											<option <?php if($result->deptid == $res_dept->sid){ echo "Selected";}?> value="<?php echo $res_dept->sid; ?>"><?php echo $res_dept->dept_name; ?></option>
									<?php	} ?>
								</select>  
							</div>
							<div class="form-group">
								<label for="input1" class="form-label">Name: </label>
								<input type="text" class="form-control" name="txtName" id="txtName" value="<?php echo $result->name?>">
							</div>
							<div class="form-group">
								<label for="input1" class="form-label">Icon: </label>
								<input type="file" class="form-control" name="txtThumb" id="txtThumb" >
								<a href='../uploades/<?php echo $result->icon?>'><?php echo $result->icon?></a>
							</div>
							<div class="form-group">
								<label for="input1" class="form-label">Page Url: </label>
								<input type="text" class="form-control" name="txturl" id="txturl" value="<?php echo $result->sulg?>">
							</div>
							<div class="form-group">
								<label for="input1" class="form-label">Order: </label>
								<input type="text" class="form-control" name="txtOrder" id="txtOrder" value="<?php echo $result->corder?>">
							</div>
