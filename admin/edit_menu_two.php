...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$sqlupdate = "UPDATE `tbl_mjcet_second_menu` SET `menuType` = '{$txtMenuType}', `menuSulg` = '{$txtUrl}', `menuName` = '{$txtMenuname}', `menuContent` = '{$txtMenuContent}', `menuOrder` = '{$txtMenuOrder}', `status` = '1'";
	...
	header('Location:menu_two.php?msg=4');
	exit();
	
}else{
	$sql = "SELECT * FROM tbl_mjcet_second_menu WHERE menuId = '{$eid}'";
...
<div class="content">
  ...
  <div class="page-header">
    <h1 class="title">Add Menu</h1>
      <ol class="breadcrumb">
	  <li><a href="menu_two.php">Menu List</a></li>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					...
					<div class="form-group">
						...
						<select class="form-control" name="txtMenuType" id="txtMenuType" >
							...
							<option value="megamenu" <?php if($result->menuType =='megamenu'){ echo 'selected'; }?>>Mega Menu</option>
						</select>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label for="input1" class="form-label">About Menu: </label>
