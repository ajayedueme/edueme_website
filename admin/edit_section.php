...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$menuNmae = htmlspecialchars(addslashes($_POST['menuNmae']));
	
		
	$sqlupdate = "UPDATE `tbl_mjcet_sections` SET `section_name`='{$txtSectionname}', `menuNmae`='{$menuNmae}', `about_section`='{$txtAbout}',`hod_title`='{$txtTitle}',`hod_introduction`='{$txtIntroduction}',`hod_description`='{$txtDescription}',`status`='1',`addOn`=now()";
	...
	if($_FILES['txtImage']['name'] !=""){
		...
		$sqlupdate .= ", section_image = '{$thumbfile2}'";
	}
	...
	$sql = "SELECT * FROM tbl_mjcet_sections WHERE sid = '{$eid}'";
	...
	?>
				...
				<form name="frmChangepassword" id="frmChangepassword" enctype="multipart/form-data"  action="" method="POST">
					<div class="form-group">
						...
						<input type="text" class="form-control" name="menuNmae" id="menuNmae" required value="<?php echo $result->menuNmae?>">
					</div>
					<div class="form-group">
						<label for="input1" class="form-label">Title: </label>
						<input type="text" class="form-control" name="txtSectionname" id="txtSectionname" required value="<?php echo $result->section_name?>">
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<a href='../uploades/<?php echo $result->section_image?>'><?php echo $result->section_image?></a>
					</div>
					<div class="form-group">
						<label for="input1" class="form-label">About Section: </label>
						<textarea class="form-control" name="txtAbout" id="elm1" required ><?php echo $result->about_section?></textarea>
					</div>
					<p>Head of the department</p>
					<div class="form-group">
						<label for="input1" class="form-label">Title: </label>
						<input type="text" class="form-control" name="txtTitle" id="txtTitle" required value="<?php echo $result->hod_title?>">
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<textarea class="form-control" name="txtIntroduction" id="elm1" required ><?php echo $result->hod_introduction?></textarea>
					</div>
					<div class="form-group">
