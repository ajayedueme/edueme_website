...
if(isset($_POST['ADD']) && $_POST['ADD'] =='category'){
	...
	$sqlupdate = "UPDATE `tbl_mjcet_second_submenu` SET `menuType` = '{$txtMenuType}', `menuSulg` = '{$txtUrl}', `menuTitle` = '{$menuTitle}', `menuName` = '{$txtMenuname}', `menuContent` = '{$txtMenuContent}', `menuContentTwo` = '{$txtMenuContent2}', `menuOrder` = '{$txtMenuOrder}', `status` = '1'";
	...
	header('Location:submenu_two.php?msg=4');
	exit();
	
}else{
	$sql = "SELECT * FROM tbl_mjcet_second_submenu WHERE menuId = '{$eid}'";
					...
					<div class="form-group">
						...
						<select class="form-control" name="txtMenuType" id="txtMenuType" >
							...
							<?php 
								...
								$mainmenu =  mysqli_query($con,"SELECT * FROM tbl_mjcet_second_menu ORDER BY menuOrder");
