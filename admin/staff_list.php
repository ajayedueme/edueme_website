...
$sqlfaq = "SELECT * FROM userlogin Order BY Dept ASC";
...
<div class="content">
  ...
  <!-- Start Page Header -->
	<div class="page-header" style="width: 100%; overflow: hidden;">
     <div style="width: 600px; float: left;"> <h1 class="title" >Staff List</h1> </div>
     <div style="float: right;"> 
	 <a class="btn btn-default" href="add_staff.php">Add</a>
		<span class="title" >Dept</span>&nbsp;&nbsp;&nbsp;&nbsp;
		<select style=" width: 150px;" id="department">
            ...
            <table id="example0" class="table display">
                ...
                <?php $i=1; while($row_faq = mysqli_fetch_object($res_faq)){?>
                    <tr>
                        ...
                        <td width="30%"><?php echo stripslashes($row_faq->FullName);?></td>
                        <td width="10%"><?php echo $row_faq->Dept;?></td>
                        <td width="30%"><?php echo $row_faq->designation;?></td>
                        <td width="10%"><?php echo $row_faq->stafforder;?></td>
						<td width="8%">
							<?php
								if($row_faq->status==1){
									$sel = "selected";
							...
							<select class="form-control" name="statchnage" >
								<option  value="1r<?= $row_faq->ULid?>" <?=$sel?> >Active</option>
								<option  value="0r<?= $row_faq->ULid?>" <?=$unsel?> >Inactive</option>
							</select>
						</td>
                        <td width="10%">
							<a href="edit_staff.php?eid=<?php echo $row_faq->ULid;?>">Edit</a> | 
							<a class="delete" href="javascript:void(0)" data-id="<?php echo $row_faq->ULid;?>">Delete</a>
						</td>
                    </tr>
                   <?php $i++; } ?> 
                </tbody>
...
$(document).ready(function() {
	...
	$('.delete').click(function(e) {
		...
		if (r == true) {
			...
			$.ajax({
				...
				type:'post',
				data:{'action':"Delete",'tblname':'userlogin','dltid':'ULid','dltval':dltid},
	...
	$('select[name^="statchnage"]').change(function(){
		...
		$.ajax({
			url:'ajax.php',
			type:'post',
			data:{'actions':'update','tablename_status':'userlogin','statuscolumn':'status','status':status_value,'idcolumn':'ULid','id':cahnge_id},
			success:function(data){
	...
	$('#department').change(function(e) {
		...
		$.ajax({
			url:'ajax.php',
			type:'post',
			data:{'action':"DeprChange",'deptid':deptid},
			success:function(data){
				$('#dept').html(data);
